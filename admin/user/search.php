...
else
{
	...
	if(!empty($_GET))
	{
		if(empty($_GET['search']) || $_GET['s'] == "" || empty($_GET['ps']))
		{
			...
			$output .= '<p>Es wurden keine User gefunden.</p>';
			$output .= '</div>';
		}
		else
		{
			if(preg_match('/[^a-zA-Z0-9\-\_\.\@]/',$_GET['search']) == 0)
			{
			...
			{
				$output .= '<div class="w3-panel w3-border w3-border-red w3-text-red">';
				$output .= '<p>Verwenden Sie nur folgende Zeichen f&uuml;r den Suchbegriff: a-z, A-Z, 0-9, -_.@</p>';
				$output .= '</div>';
			}
		}
	}
...
<html lang="de">
	<head>
		<title>WebBar | Admin | User suchen</title>
		<?php
