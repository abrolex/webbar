...
else
{
	...
	if(!empty($_GET))
	{
		if($_GET['s'] == "" || empty($_GET['ps']))
		{
			...
			$output .= '<p>Es konnten keine Artikel angezeigt werden.</p>';
			$output .= '</div>';
		}
		else
		{
						...
						else
						{
							...
							if($anzahl_gs > 0)
							{
								...
								while($row = $result->fetch_array(MYSQLI_ASSOC))
								{
									...
									$output .= '<a class="w3-btn w3-block blue" href="view.php?article_id='.$row['article_id'].'"><i class="fas fa-arrow-right"></i></a>';
									$output .= '</div>';
									$output .= '</div>';
								}
							...
							{
								$output .= '<div class="w3-panel w3-border w3-border-red w3-text-red">';
								$output .= '<p>Es wurden noch keine Artikel erstellt.</p>';
								$output .= '</div>';
							}
						}
					}
